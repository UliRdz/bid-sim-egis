...
function createLogoFrames(officeWidth, officeDepth, wallHeight) {
    ...
    // Logo file paths - try multiple extensions and formats
    const logoFiles = [
        './assets/logo-1.png',
        './assets/logo-2.png',
        './assets/logo-3.png',
    ...
    function createFrame(position, rotation, logoPath, frameNumber) {
        ...
        function createPlaceholder(showError = false) {
            ...
            context.fillText('FRAME ' + frameNumber, 256, 200);
            
            context.font = '32px Inter, sans-serif';
            context.fillStyle = '#007C91';
            if (showError) {
                context.fillText('Image not found', 256, 270);
                context.font = '20px Inter, sans-serif';
                context.fillText('Place logo here:', 256, 310);
                context.fillText(logoPath, 256, 340);
            } else {
                context.fillText('Add your logo:', 256, 270);
                context.fillText(logoPath.split('/').pop(), 256, 310);
            }
            ...
            const oldPlane = frameGroup.children.find(child => 
                child.geometry && child.geometry.type === 'PlaneGeometry'
            );
            if (oldPlane) {
                frameGroup.remove(oldPlane);
            }
        ...
        // Set crossOrigin for GitHub Pages
        ...
        textureLoader.load(
            ...
            (texture) => {
                ...
                frameGroup.add(logoPlane);
                console.log(`✓ Loaded logo: ${logoPath}`);
            },
            undefined,
            (error) => {
                // Error loading - update placeholder to show error
                createPlaceholder(true);
                console.log(`⚠ Logo not found: ${logoPath} - showing placeholder`);
            }
        );
